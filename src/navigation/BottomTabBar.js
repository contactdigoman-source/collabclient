...
import { createTableForAttendance } from '../services/attendance-db-service';
...
export default function BottomTabBar() {
  ...
  const { userLastAttendance } = useAppSelector(state => state.userState);
  ...
  const onPunchButtonLongPress = useCallback(async () => {
  ...
  const getAttendanceIcon = useMemo(
    () =>
      isUserCheckedIn
    ...
    (routeName, selectedTab) => {
      ...
      return (
        <View style={styles.centered}>
          <AppImage size={hp('2.48%')} source={icon} style={{ tintColor }} />
