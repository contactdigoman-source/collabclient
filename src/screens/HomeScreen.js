...
export default function HomeScreen() {
  ...
  const lastAttendanceCoords = useMemo(() => {
  ...
  const sections = useMemo(
    () => [
      // {
      //   title: 'Recent Colab',
      //   data: Array(2).fill({ dummy: true }),
      //   layout: SECTION_LIST_LAYOUTS.recentColab,
      // },
      {
        title: 'Colleagues',
        data: Array(4).fill({ dummy: true }),
  ...
  const renderGridSection = useCallback(
    (data, layout, numColumns, itemStyle) => (
      <FlatList
        data={data}
